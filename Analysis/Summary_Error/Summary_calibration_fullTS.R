...
### 3. Famous - Estimated Variance
load("Official_Fast/output_rep_var/mhParameters_6.RData")
famosOutput_rep_var<-matrix(NA,
                            nrow=length(initResultsList[[2]]), 
                            ncol=length(initResultsList[[2]][[1]][[2]]), byrow = TRUE)
for(k in 1:length(initResultsList[[2]])){
  famosOutput_rep_var[k,]<-initResultsList[[2]][[k]][[2]]
}
...
names(handTuneOutput)<-colnames(precalibrationOutput_MSE)<-colnames(precalibrationOutput_Window)<-colnames(famosOutput)<-colnames(famosOutput_rep_var)<-dateVect

save(dateVect , subsetFinalObs , famosOutput_rep_var , 
     famosOutput , precalibrationOutput_MSE , precalibrationOutput_Window,
     handTuneOutput,
     file="manuscript/revisionCode/resultsStremflow_calibration_full.RData") 
