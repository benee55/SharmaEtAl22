...
newIntervalDate<-data.frame(as.Date(intervalMat[,1],format="%Y%m%d"), as.Date(intervalMat[,2],format="%Y%m%d"))
colnames(newIntervalDate)<-c("start","end")
newIntervalDate
...
for(i in 1:5){
  legend("topright", legend=c("Observations", "Selected Events" , "Action Stage" ,"Model Runs"), 
         pch=c(16,16,NA,NA), 
         lty=c(NA,NA,1,NA),
         fill= c(NA,NA,NA,"gray"),border = c(NA,NA,NA,gray),
         col=c("black","red","blue","gray"))  
}
for(i in 1:nrow(newIntervalDate)){
  polygon(x=c(newIntervalDate[i,1],newIntervalDate[i,2],
              newIntervalDate[i,2],newIntervalDate[i,1]),
          y=c(-1000,-1000,4950.55,4950.55), col = rgb(190,190,190,alpha=150 , maxColorValue = 255),  border = rgb(190,190,190,alpha=100 , maxColorValue = 255))
}
...
load("preCalibrationResults.RData") # Load randomly selected sample
...
for(i in 1:21){
  ...
  points(x=i, y=mean(outputMat[,i]), col="green" ,pch=16)
}
abline(h=4950.55, col="blue" ,lwd=2) # ACtion Stage
legend("topright", legend=c("Observation" , "Action Stage" , "Ensemble Mean"), 
       pch=c(16,NA,16),  
       lty=c(NA,1,NA) , 
       col=c("red","blue","green"))  

par(mar=c(6,4,2,2))
vioplot(outputMat  , las=2 , main="Model Output and observations" , 
        colMed = "green")
for(i in 1:21){
  points(x=i, y=obs[i], col="red" ,pch=16 , cex=1)
}
abline(h=4950.55, col="blue" ,lwd=2) # ACtion Stage
legend("topright", legend=c("Observation" , "Action Stage" , "Ensemble Mean"), 
       pch=c(16,NA,16),  
