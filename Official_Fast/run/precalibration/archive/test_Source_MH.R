...
logPosterior_var<-function(par, priorPar, obs , output){
  if(par<0){
    return(-1e10)
  }else{
    lPrior<-dinvgamma(x=par, shape = priorPar[1] , rate = priorPar[2] , log = TRUE)
    llhd<-sum(dnorm(x=obs , mean = output , sd = sqrt(par) , log = TRUE))    
    return(lPrior + llhd)
  }
}
#######################################################################################################
...
mcmcManual_tempered<-function(iter,
  ...
  for(i in 2:iter){
    ...
    candMat[i,]<-c(rnorm(n=1 , mean = parMat[i-1,1] , sd = sqrt(propSigma2)) , 
                   rtmvnorm(n = 1,mu = parMat[i-1,-1], 
                            sigma = propCov,
    ...
    alphaMat[i,2:3]<-c(rep(exp(alpha),2)) # Gibbs sampling for variance and exp(alpha) for the thetas
    ...
    num<-logPosterior_var(par=candMat[i,1] , priorPar = priorPar[1,] , obs = obs , output=resultsList[[i]])
    den<-logPosterior_var(par=parMat[i-1,1] , priorPar = priorPar[1,] , obs = obs , output=resultsList[[i]])
    alpha<-min(num-den, log(1))
    if(alpha>log(runif(1))){parMat[i,1]<-candMat[i,1]}else{parMat[i,1]<-parMat[i-1,1]}
    alphaMat[i,1]<-exp(alpha)
    # parMat[i,1]<-obsVar
  }
